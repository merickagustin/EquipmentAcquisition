...
(async () => {
  ...
  console.log('=== Create a purchase order against it ===');
  await page.getByRole('button', { name: 'New Purchase Order' }).click();
  await page.getByLabel('Acquisition Request Id').last().waitFor({ timeout: 10000 });
  await page.getByLabel('Acquisition Request Id').last().fill(String(target));
  await page.getByLabel('Acquisition Request Id').last().blur();
  await page.waitForSelector('text=status Approved', { timeout: 10000 });
  console.log('Request lookup confirmed Approved.');
